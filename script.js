document.addEventListener("DOMContentLoaded", function() {
    ...
    function showUser() {
      ...
      userInfo.innerHTML = `${username}, ${balance}`; // Исправлено: Используйте обратные кавычки для интерполяции строк
    ...
    let gameButton = document.querySelector(".gameButton"); // Исправлено: используйте querySelector
    gameButton.addEventListener("click", startOrStopGame); // Исправлено: используйте правильный метод для добавления события
      ...
      for (let row = 0; row < table.length; row++) {
        for (let column = 0; column < table[row].length; column++) {
          ...
          if (value === 0) {
            ...
            cells[j].classList.remove("active");
            cells[j].classList.remove("flag");
            cells[j].innerHTML = value; // Устанавливаем цифру
          } else if (value === "BOMB") {
            cells[j].classList.remove("active");
            cells[j].classList.remove("flag");
